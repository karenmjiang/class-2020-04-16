...
```
When income changes by one unit, the likelihood for voting
...
```{r}
tibble(`Error rate` = vote_forest$fit$err.rate[, "OOB"], Trees = 1:500) %>%
...
vote_logistic <- fit(logistic_mod,
                     vote ~ ideology + income + gender + race,
                     data = nes)

vote_cart <- fit(cart_mod, 
                 vote ~ ideology + income + gender + race,
                 data = nes)
...
predictions <- nes %>%
  mutate(logistic = predict(vote_logistic, new_data = nes) %>%
           pull(.pred_class),
         cart = predict(vote_cart, new_data = nes) %>% 
           pull(.pred_class),
         forest = predict(vote_forest, new_data = nes) %>%
           pull(.pred_class)
  )

predictions %>% sample_n(5)

tibble(logistic = mean(predictions$logistic == predictions$vote),
       cart = mean(predictions$cart == predictions$vote),
